...
- Add this to **Framework Search Path**
    ...
    ![](./Docs/KMM/Images/frameworkSearchPath)
...
And that is by adding new library - `KMP-NativeCoroutines`:
 ...
 - This needs to be added both into KMM library and in Xcode project
    ...
    - Also add `kotlin.sourceSets.all` under the `sourceSets` in *`build.gradle.kts`*
    ...
    - After that, add `@NativeCoroutines` to your function, and it will automatically generate new use case thats thread safe.
    ...
    - After that you need to use specific functions from **KMPNativeCoroutines** library, for our purpose i chose the Async version and it looks like this:
        
        `asyncFunction(for: **“YourModule.yourFunction()”**)`
